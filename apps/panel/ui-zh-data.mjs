// 由 apps/control-center/src/ui-zh.ts 自动生成，请勿手工编辑。
// 重新生成：node apps/control-center/scripts/build-ui-zh-data.mjs
// 简体中文界面覆盖层。
//
...
function toChineseDate(month, day, year) {
...
function localizeDateText(input) {
  let out = input;
  out = out.replace(
    new RegExp(`\\b(${EN_MONTH_RE})\\s+(\\d{1,2}),\\s*(\\d{4})\\b`, "g"),
    (_m, mon, day, year) => toChineseDate(mon, day, year),
  );
  out = out.replace(
    new RegExp(`\\b(${EN_MONTH_RE})\\s+(\\d{1,2})\\b`, "g"),
    (_m, mon, day) => toChineseDate(mon, day),
  );
  out = out.replace(
    new RegExp(`\\b(${EN_MONTH_RE})\\s+(\\d{4})\\b`, "g"),
    (_m, mon, year) => `${year}年${EN_MONTHS[mon]}月`,
  );
  out = out.replace(
    /\b(\d{1,2}):(\d{2})\s*(AM|PM)\b/gi,
    (_m, hour, minute, period) => to24Hour(hour, minute, period),
  );
  out = out.replace(
    /\b(\d{1,2})\s*(AM|PM)\b/gi,
    (_m, hour, period) => to24Hour(hour, "00", period),
  );
...
export function translateZhText(raw) {
  if (!raw) return null;
